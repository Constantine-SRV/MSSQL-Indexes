...
--delete @deletePercentExceedingLimit old records if table size > than @maxTbSizeMb
                ...
                begin
                               ...
                               ALTER INDEX [PK_tbl_WhoIsActive] ON [AdminTools].[dbo].tbl_IndexStatusNorm REBUILD PARTITION = ALL WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON)
                end
...
--executing and logging
		...
		BEGIN TRY
			...
			update  [AdminTools].[dbo].[tbl_IndexStatusNorm] set dtf=getdate(),[vcRes]='OK' where iIndexStatusID= @iIndexStatusNormID
			...
			begin -- was reorganise and will do statistic update
				...
				RAISERROR (@sql, 0, 1) WITH NOWAIT
				EXEC sys.sp_executesql @sql
				update  [AdminTools].[dbo].[tbl_IndexStatusNorm] set dtf=getdate(),[vcRes]='OK',[cAct]='S' where iIndexStatusID= @iIndexStatusNormID
			end
		END TRY
		BEGIN CATCH
			...
			update  [AdminTools].[dbo].[tbl_IndexStatusNorm] set dtf=getdate(),[vcRes]=ERROR_MESSAGE() where iIndexStatusID= @iIndexStatusNormID
		END CATCH;
		RAISERROR (@msgTxt, 0, 1) WITH NOWAIT
end
else
