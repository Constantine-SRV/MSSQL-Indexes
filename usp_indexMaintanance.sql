...
CREATE TABLE #indSt (
    ...
    ds_Type varchar(50),
    iAttempt int NOT NULL DEFAULT 1,        -- номер попытки, начинаем с 1
    [ID] [int] IDENTITY(1,1)
)
...
WHERE i.[type] IN (1, 2)
    ...
    AND o.[type] IN (''U'', ''V'')
ORDER BY s.avg_fragmentation_in_percent DESC  -- сначала самые фрагментированные
'

-- iAttempt не включаем — DEFAULT 1 подставится автоматически
...
DECLARE
    ...
    @iAttempt int                           -- текущий номер попытки
...
BEGIN
    ...
    BEGIN
        ...
        VALUES (@DB_ID, @indexID, @SchemaID, @TableID, @page_count,
                ...
                getdate(), @JobID, IIF(@rebuild=1,'B','O'),
                'start (attempt ' + CAST(@iAttempt AS varchar) + ')')   -- фиксируем номер попытки
        ...
        BEGIN CATCH
            SET @msgTxt = CONVERT(varchar, @id) + ' from ' + CONVERT(varchar, @idmax) +
                          ' pages:' + CONVERT(varchar, @page_count) + ' ' +
                          CONVERT(varchar, getdate(), 120) + ' attempt:' + CAST(@iAttempt AS varchar) +
                          ' ' + ERROR_MESSAGE()

            UPDATE [admintools].[dbo].[tbl_IndexStatusNorm]
                SET dtf = getdate(),
                    [vcRes] = 'attempt ' + CAST(@iAttempt AS varchar) + ': ' + ERROR_MESSAGE()
            WHERE iIndexStatusID = @iIndexStatusNormID

            -- Повторная попытка: добавляем в конец #indSt, если лимит не исчерпан
            IF @iAttempt <= @MaxRetries
            BEGIN
                INSERT INTO #indSt (indexID, indexName, Schemaid, SchemaName, TableID, TableName,
                    avg_fragmentation_in_percent, page_count, is_lob_legacy, is_lob,
                    partition_number, ds_Type, iAttempt)
                VALUES (@indexID, @indexName, @SchemaID, @SchemaName, @TableID, @TableName,
                    @avg_fragmentation_in_percent, @page_count, @is_lob_legacy, @is_lob,
                    @partition_number, @ds_Type, @iAttempt + 1)

                SET @msgTxt = @msgTxt + ' --> retry scheduled (attempt ' +
                              CAST(@iAttempt + 1 AS varchar) + ')'
            END
        END CATCH
